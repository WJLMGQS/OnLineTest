...
public class Paper_list_status extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(showMessage==null){//数据检测通过后更新试卷中的数据信息
			...
			List<TestPaper> allTestPapers = testPaperServiceImp.getAllTestPaperByAdminId(teacher,statusTrack);
			request.setAttribute("allTestPapers", allTestPapers);
			request.setAttribute("paperStatus", statusTrack);
			//转向显示所有试卷的页面
			request.getRequestDispatcher("/teacher/admin/paper_list_status.jsp").forward(request,response);
		}else{
