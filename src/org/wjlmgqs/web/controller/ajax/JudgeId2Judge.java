...
/**
 */
public class JudgeId2Judge extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String judgeId = request.getParameter("opvalue").trim();
		PrintWriter out = response.getWriter();
		String responseContext = "{}";
		// 设置要输出的日期默认格式
		final String dataPartten = "yyyy-MM-dd HH:mm:ss";
		// 设置要输出的属性
		final String regFormat = "|result|image|analyse|content|difficulty|createTime|";
		...
		Gson gson = new GsonBuilder().setDateFormat(dataPartten)
				.setExclusionStrategies(new ExclusionStrategy() {
					...
					public boolean shouldSkipField(FieldAttributes attr) {
						...
						// 当遇到要过滤的类时
						if (attr.getDeclaringClass() == TestUnit.class
								|| attr.getDeclaringClass() == Judge.class) {// 这里表示字段声明时所在的类：attr.getDeclaringClass()
		...
		if (judgeId == null) {
			...
			try {
				...
				// 返回所有记录
				judge = judgeServiceImp.getJudgeById(judge_id,(Teacher) request.getSession().getAttribute("professionTeacher"));
			} catch (NumberFormatException e) {
				WebLogger.showInfor("传入的知识点参数错误！");
				e.printStackTrace();
			}
		}
		responseContext = gson.toJson(judge);
		// 替换难度显示符号
		try {
