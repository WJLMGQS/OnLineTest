...
/**
 */
public class Knowledge2fillBlanks extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<FillBlank> fillBlanks = null;
		// 设置要输出的日期默认格式
		...
		Map<String, List<FillBlank>> map = new HashMap<String, List<FillBlank>>();
		if (knowledgeId == null) {
		} else {
			try {
				...
				// 返回所有记录
				fillBlanks = fillBlankServiceImp.getAllFillBlanksByKnowledgeId(knowledge_id, (Teacher) request.getSession().getAttribute("professionTeacher"));
			} catch (NumberFormatException e) {
				WebLogger.showInfor("知识点编号错误");
				e.printStackTrace();
			}
		}
		map.put("aaData", fillBlanks);
