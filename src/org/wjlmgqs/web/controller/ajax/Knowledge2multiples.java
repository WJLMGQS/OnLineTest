...
public class Knowledge2multiples extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Map<String,List<Multiple>> map = new HashMap<String,List<Multiple>>();
		if (knowledgeId == null) {
		} else {
			try {
				...
				//返回所有记录
				multiples = multipleServiceImp.getAllMultiplesByKnowledgeId(knowledge_id,(Teacher)request.getSession().getAttribute("professionTeacher"));
			} catch (NumberFormatException e) {
				WebLogger.showInfor("知识点编号错误");
				e.printStackTrace();
			}
		}
		map.put("aaData",multiples);
