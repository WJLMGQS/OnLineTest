	...
	<%-- 图片放大显示插件--%>
 	...
 	<script type="text/javascript">
		$(document).ready(function() {
			//初始化图片href和src参数，显示图片
			var imageUrl = "${pageContext.request.contextPath}/${initParam.questionBankFolder}/${fillBlank.image}?random=" + (new Date()).getTime();
			...
			$("#uploadify").uploadify({
			   ...
			   'script': '${pageContext.request.contextPath}/teacher/profession/questionBank_fillBlank_update_image',//*要提交到的处理文件上传后台文件
			   'scriptData':{'fillBlankId':$('#fillBlankId').val()},//*
	           'method':'GET',
			...
			addOrDeleteFillBlankRow('${fn:length(fillBlank.results)}');//动态删除、添加选项行,下标从选项长度开始
		});		
	</script>
  </head>
  <body>
 	<center><h1>填空试题信息更改</h1>
 	<a href="${pageContext.request.contextPath}/teacher/profession/questionBank_fillBlank">后退</a></center>
<hr/>
<table>
<tr>
<td>
	<a id="floatImage" target="_blank" class="preview" title="填空试题图片">
...
<td> <center>
	<input type="hidden" value="${fillBlank.id }" id="fillBlankId"/>
	<form id="infoForm" action="${pageContext.request.contextPath}/teacher/profession/questionBank_fillBlank_update_info?fillBlankId=${fillBlank.id}" method="post">
		文本内容 <textarea rows="6" cols="60"   name="content" reg="\S+" tip="不能为空！">${fillBlank.content }</textarea><br/>
	   	<input id="BtAdd" type="button" value="添加" /> 
		<input id="BtDel" type="button" value="删除" /> 
		<input id="BtIne" type="button" value="分割" /> 
		选项<table id="checkBoxTable" border='1'>
				<tr><!--这一行是标题行，下标为0-->
					<td></td><!--下标为0-->
					<td>全选<input id="CKA" type="checkbox" /></td>
					<td>空格内容(注:可以使用░来分割多个答案对应同一个空格的情况。)</td>
				</tr>
				<tr ><!--这一行是模板行，用来克隆其它行，并隐藏,下标为1-->
					<td></td>
					<td><input id="CK" type="checkbox"/></td>
					<td><input id="TName" type="text" name="results"/></td>
				</tr>
				<c:forEach items="${requestScope.fillBlank.results}" var="result" varStatus="status" step="1" >
					<tr>
						<td>${status.count}</td>
						<td><input id="CK" type="checkbox"/></td>
						<td><input id="TName" type="text" name="results" value='${requestScope.fillBlank.results[status.count-1] }'/></td>
					</tr>
				</c:forEach>
			</table>
	   	难度<select name="difficulty" size="1" tip="请选择难度" reg="[0|1|2|3|4]+">
						<option value="0" ${fillBlank.difficulty.index=='0'?'selected':'' }>简单</option>
						<option value="1" ${fillBlank.difficulty.index=='1'?'selected':'' }>较简单</option>
						<option value="2" ${fillBlank.difficulty.index=='2'?'selected':'' }>中等</option>
						<option value="3" ${fillBlank.difficulty.index=='3'?'selected':'' }>较困难</option>
						<option value="4" ${fillBlank.difficulty.index=='4'?'selected':'' }>困难</option>
			</select>          
	  	知识点<select name="knowledgeId" size="1" tip="请选择知识点" reg="[1-9]+">
					<c:forEach items="${requestScope.knowledges}" var="knowledge" step="1">
						<option value="${knowledge.id}" }  ${fillBlank.knowledge.id==knowledge.id?'selected':''}>${knowledge.code}</option>
					</c:forEach>
			</select>     
		解析<textarea rows="3" cols="25" name="analyse" reg="\S+" tip="不能为空！">${fillBlank.analyse }</textarea><br/>
		<br/>
		<input id="updateInfo" type="submit" value="更新" onclick="return validateFillBlankFormBlank();"/>&nbsp;&nbsp;<input type="reset" value="重置"/>
	</form>
	<hr/>		
 </center></td>
	</tr>
