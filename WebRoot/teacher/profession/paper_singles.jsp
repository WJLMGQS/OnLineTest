...
<html>
 ...
 <body>
 <center>
 	<h1>${professionTeacher.subject.code}-单选题</h1>
 	<a href="${pageContext.request.contextPath}/teacher/profession/paper_singles_add">添加新试题</a>
...
<hr/>
	...
	<c:forEach items="${singles}" var="single" varStatus="status" step="1" >
		<div id="showSingleItem" class="defineTop" >
			<div><h4 align="center">第${status.count}题</h4></div>
			<div class="defineBase">
				<div>	
					<fieldset>
						<legend>内容：</legend>
						<textarea rows="4" cols="55" id="content" disabled="disabled">${single.content}</textarea>
					</fieldset>	
				</div>	
				<div>	选项A：<input id="SA" type="text" value="${single.SA}" disabled="disabled"/>&nbsp;&nbsp;选项B：<input id="SB" type="text" value="${single.SB}" disabled="disabled"/></div>	
				<div>	选项C：<input id="SC" type="text" value="${single.SC}" disabled="disabled"/>&nbsp;&nbsp;选项D：<input id="SD" type="text" value="${single.SD}" disabled="disabled"/></div>			
				<div>	答案：<input id="result" type="text" size="1" value="${single.result }" disabled="disabled"/>&nbsp;&nbsp;难度：<input id="difficulty" value="${single.difficulty.name}" type="text" size="2" disabled="disabled"/>
									&nbsp;&nbsp;
								创建时间：<fmt:formatDate value="${single.createTime}" pattern="yy/MM/dd HH:mm:ss"/></div>	
				<div>	
					<fieldset>
						<legend>分析：</legend>
						<textarea id="analyse" rows="4" cols="55" disabled="disabled">${single.analyse}</textarea>
					</fieldset>	
				</div>	
