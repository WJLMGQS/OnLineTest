...
<html>
 ...
 <center>
 	...
 	<!-- 收缩展开效果 -->
	<div class="hiddenDiv">
		<div class="hiddenTitle"><h3>创建新知识点</h3></div>
		<div class="hiddenContent">
			<form action="${pageContext.request.contextPath}/teacher/profession/name_knowledge_create" method="post">
			  	知识点代码<input type="text" name="code" tip="不能为空！" reg="\S+"/>
...
<hr/>
	<table cellpadding="5" cellspacing="0" border="1" class="display" id="eachRow" style="text-align: center" >
		<thead>
			<tr><th>序号</th><th>知识点代码</th><th>操作</th></tr>
		</thead>
		<tbody>
			<c:forEach items="${knowledges}" var="knowledge" varStatus="status" step="1" >
				<tr>
					<td>${status.count}</td> 
					<td>${knowledge.code}</td> 
					<td><a href="###" onclick="showConfirmDialog('name_knowledge_delete?knowledgeId=${knowledge.id}')" title="删除"><img src="${pageContext.request.contextPath}/images/icons/cross.png" alt="删除"/></a>
						&nbsp;&nbsp;
						<a href="${pageContext.request.contextPath}/teacher/profession/name_knowledge_update?knowledgeId=${knowledge.id}" title="编辑"><img src="${pageContext.request.contextPath}/images/icons/pencil.png" alt="编辑"/></a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
