 ...
 <center>
	...
	<div class="hiddenDiv">
		...
		<div class="hiddenContent">
			<form id="submitForm" action="${pageContext.request.contextPath}/teacher/profession/questionBank_fillBlank_create" method="post" >
				...
				<br/>
				<input type="submit" value="创建" onclick="return validateFillBlankFormBlank();"/>
			</form>
		</div>
	</div>
<hr/>
	<table cellpadding="5" cellspacing="0" border="1" class="display" id="eachRow" style="text-align: center" >
		...
		<tbody>
			<c:forEach items="${fillBlanks}" var="fillBlank" varStatus="status" step="1" >
				<tr>
					<td>${status.count}</td> 
					<td>${fillBlank.content}</td> 
					<td>${fillBlank.knowledge.code}</td> 
					<td>${fillBlank.difficulty.name}</td> 
					<td><fmt:formatDate value="${fillBlank.createTime}" pattern="yy/MM/dd HH:mm:ss"/></td> 
					<td><a href="###" onclick="showConfirmDialog('questionBank_fillBlank_delete?fillBlankId=${fillBlank.id}')" title="删除"><img src="${pageContext.request.contextPath}/images/icons/cross.png" alt="删除"/></a>
						&nbsp;&nbsp;
						<a href="${pageContext.request.contextPath}/teacher/profession/questionBank_fillBlank_update?fillBlankId=${fillBlank.id}" title="编辑"><img src="${pageContext.request.contextPath}/images/icons/pencil.png" alt="编辑"/></a>
					</td>
