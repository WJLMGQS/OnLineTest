 ...
 <center>
	...
	<div class="hiddenDiv">
		...
		<div class="hiddenContent" >
			<form action="${pageContext.request.contextPath}/teacher/profession/questionBank_single_create" method="post">
			  	文本内容 <textarea rows="6" cols="60" name="content" reg="\S+" tip="不能为空！"></textarea><br/>
			   	选项A<input type="text" name="SA" width="180" reg="\S+" tip="不能为空！"/>&nbsp;&nbsp;
			  	选项B<input type="text" name="SB" width="180" reg="\S+" tip="不能为空！"/><br/>
				选项C<input type="text" name="SC" width="180" reg="\S+" tip="不能为空！"/>&nbsp;&nbsp;         
				选项D<input type="text" name="SD" width="180" reg="\S+" tip="不能为空！"/><br/>               
			   	选项答案:	
			   		A<input type=radio name="result" value="A" checked="checked"/>  &nbsp;&nbsp;
			   		B<input type=radio name="result" value="B"/>  &nbsp;&nbsp;
			   		C<input type=radio name="result" value="C"/>  &nbsp;&nbsp;
			   		D<input type=radio name="result" value="D"/>  &nbsp;&nbsp;  <br/>      
...
<hr/>
	<table cellpadding="5" cellspacing="0" border="1" class="display" id="eachRow" style="text-align: center" >
		<thead>
			<tr>
				<th>序号</th><th>文本内容</th><th>知识点</th><th>难度</th><th>修改时间</th><th>操作</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${singles}" var="single" varStatus="status" step="1" >
				<tr>
					...
					<td>${single.knowledge.code}</td> 
					<td>${single.difficulty.name}</td> 
					<td><fmt:formatDate value="${single.createTime}" pattern="yy/MM/dd hh:mm:ss"/></td> 
					<td><a href="###" onclick="showConfirmDialog('questionBank_single_delete?singleId=${single.id}')" title="删除"><img src="${pageContext.request.contextPath}/images/icons/cross.png" alt="删除"/></a>
						&nbsp;&nbsp;
						<a href="${pageContext.request.contextPath}/teacher/profession/questionBank_single_update?singleId=${single.id}" title="编辑"><img src="${pageContext.request.contextPath}/images/icons/pencil.png" alt="编辑"/></a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
