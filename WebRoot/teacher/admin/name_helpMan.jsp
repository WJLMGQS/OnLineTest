...
<html>
 ...
 <center>
	...
	<!-- 收缩展开效果 -->
	<div class="hiddenDiv">
		<div class="hiddenTitle"><h3>创建新辅导员</h3></div>
		<div class="hiddenContent">
			<form action="${pageContext.request.contextPath}/teacher/admin/name_helpMan_create" method="post">
				辅导员名称<input type="text" name="code" reg="\S+" tip="不能为空！" />
				学院名称<select name="departmentId" size="1" tip="请选择学院" reg="[1-9]+">
...
<hr/>
	<table cellpadding="5" cellspacing="0" border="1" class="display" id="eachRow" style="text-align: center" >
		<thead>
			<tr><th>序号</th><th>辅导员名称</th><th>所属学院</th><th>操作</th></tr>
		</thead>
		<tbody>
			<c:forEach items="${helpMans}" var="helpMan" varStatus="status" step="1"  >
				<tr>
					<td>${status.count}</td>
					<td>${helpMan.code}</td>
					<td>${helpMan.department.code}</td>
					<td><a href="###" onclick="showConfirmDialog('name_helpMan_delete?helpId=${helpMan.id}')" title="删除"><img src="${pageContext.request.contextPath}/images/icons/cross.png" alt="删除"/></a>
						&nbsp;&nbsp;
						<a href="${pageContext.request.contextPath}/teacher/admin/name_helpMan_update?helpId=${helpMan.id}" title="编辑"><img src="${pageContext.request.contextPath}/images/icons/pencil.png" alt="编辑"/></a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
