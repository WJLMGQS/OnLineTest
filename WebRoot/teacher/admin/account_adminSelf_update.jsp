...
<html>
 ...
 <h1>管理员账号信息</h1>
  ${refreshLeftMenu}
  	教师编号-${sessionScope.adminTeacher.userId}
<hr/>
	<form id="infoForm" action="${pageContext.request.contextPath}/teacher/admin/account_adminSelf_update_info" method="post">
		姓名<input id="name" name="name" type="text" value="${sessionScope.adminTeacher.name}" reg="\S+" tip="不能为空！"/>
		&nbsp;&nbsp;
		性别<input type="radio" name="sex" value="1" ${sessionScope.adminTeacher.sex=='1'?"checked":''}/>男
			 <input type=radio name="sex" value="0" ${sessionScope.adminTeacher.sex=='0'?"checked":''}/>女
		&nbsp;&nbsp;
		电话<input id="telephone" name="telephone" type="text" value="${sessionScope.adminTeacher.telephone}" reg="[\d-]+" tip="由数字和-组成,不能为空！"/>
		<input id="updateInfo" type="submit" value="更新" onclick="clearPwd()"/>
		&nbsp;&nbsp;
		<input type="reset" value="重置"/>
	</form>
<hr/>
	<form id="pwdForm" action="${pageContext.request.contextPath}/teacher/admin/account_adminSelf_update_pwd" method="post">
		原密码<input id="original" name="original" type="password" reg="\w{5,}" tip="密码至少5位，由数字/字母/下划线组成！"/><br/>
		新密码<input id="freshF" name="freshF"  type="password" reg="\w{5,}" tip="密码至少5位，由数字/字母/下划线组成！"/>
		&nbsp;&nbsp;
		确认新密码<input id="freshS" name="freshS" type="password" reg="\w{5,}" tip="请确保与新密码输入一致！"/>
		<input id="updatePwd" type="submit" value="更新"/>
		&nbsp;&nbsp;
