...
<html>
 ...
 <center>
	...
	<!-- 收缩展开效果 -->
	<div class="hiddenDiv">
		<div class="hiddenTitle"><h3>创建新班级</h3></div>
		<div class="hiddenContent">
			<form action="${pageContext.request.contextPath}/teacher/admin/name_classes_create" method="post">
				班级名称<input type="text" name="code" reg="\S+" tip="不能为空！" /><br/>
				学院名称<select name="departmentId" id="departmentId" size="1" onchange="back2Default()" tip="请选择学院" reg="[1-9]+">
							<option value="0" selected>----请选择以下学院----</option>
				...
				年级名称<select name="gradeId" id="gradeId" size="1" onchange="back2Default()"  tip="请选择年级" reg="[1-9]+">
							<option value="0" selected>----请选择以下年级----</option>
				...
				&nbsp;&nbsp;
			   	专业名称<select name="careerId" id="careerId" size="1"  tip="请选择专业" reg="[1-9]+">
...
<hr/>
	<table cellpadding="5" cellspacing="0" border="1" class="display" id="eachRow" style="text-align: center" >
		<thead>
			<tr><th>序号</th><th>名称</th><th>专业</th><th>年级</th><th>学院</th><th>辅导员</th><th>辅导员所属学院</th><th>操作</th></tr>
		</thead>
		<tbody>
			<c:forEach items="${classess}" var="classes" varStatus="status" step="1" >
				<tr>
					<td>${status.count}</td>
					<td>${classes.code}</td>
					<td>${classes.career.code}</td><td>${classes.career.grade.code}</td>
					<td>${classes.career.department.code}</td><td>${classes.helpMan.code}</td>
					<td>${classes.helpMan.department.code}</td>
					<td><a href="###" onclick="showConfirmDialog('name_classes_delete?classId=${classes.id}')" title="删除"><img src="${pageContext.request.contextPath}/images/icons/cross.png" alt="删除"/></a>
						&nbsp;&nbsp;
						<a href="${pageContext.request.contextPath}/teacher/admin/name_classes_update?classId=${classes.id}" title="编辑"><img src="${pageContext.request.contextPath}/images/icons/pencil.png" alt="编辑"/></a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
