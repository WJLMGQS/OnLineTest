...
<html>
 ...
 <center>
	...
	<!-- 收缩展开效果 -->
	<div class="hiddenDiv">
		<div class="hiddenTitle"><h3>创建新科目</h3></div>
		<div class="hiddenContent">
			<form action="${pageContext.request.contextPath}/teacher/admin/name_subject_create" method="post">
				科目名称<input type="text" name="code" reg="\S+" tip="不能为空！" />
...
<hr/>
	<table cellpadding="5" cellspacing="0" border="1" class="display" id="eachRow" style="text-align: center" >
		<thead>
			<tr><th>序号</th><th>科目名称</th><th>科目操作</th></tr>
		</thead>
		<tbody>
			<c:forEach items="${subjects}" var="subject" varStatus="status" step="1" >
				<tr>
					<td>${status.count}</td>
					<td>${subject.code}</td>
					<td>
						<a href="###" onclick="showConfirmDialog('name_subject_delete?subjectId=${subject.id}')" title="删除"><img src="${pageContext.request.contextPath}/images/icons/cross.png" alt="删除"/></a>
						&nbsp;&nbsp;
						<a href="${pageContext.request.contextPath}/teacher/admin/name_subject_update?subjectId=${subject.id}" title="编辑"><img src="${pageContext.request.contextPath}/images/icons/pencil.png" alt="编辑"/></a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
