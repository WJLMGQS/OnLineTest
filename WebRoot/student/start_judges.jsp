	...
	<c:forEach items="${judges}" var="judge" varStatus="status" step="1" >
		<div id="showSingleItem" class="defineTop" tip="${status.count}">
			...
			<div class="defineTitle">
					...
					<c:choose>
						<c:when test="${judge.image=='defaultQuestBank.jpg'}">[当前试题无图]</c:when>
						<c:otherwise>
			...
			<div class="defineBase">
				<div>	
					<fieldset>
						<legend>内容：</legend>
						<textarea rows="4" cols="75" id="content" disabled="disabled">${judge.content}</textarea>
					</fieldset>	
				</div>	
				<div class="defineResult" title="正确">
					T<input value="T" name="judge_${status.count-1}" type="radio"  ${judge.result=='T'?'checked':''}/>
						&nbsp;&nbsp;
					F<input value="F" name="judge_${status.count-1}" type="radio"  ${judge.result=='F'?'checked':''}/>
				</div>	
			</div>
		</div>
		<br/><br/>
	</c:forEach>
			<div><a href="###" onclick="isSaveJudge('2')" title="保存">保存试题</a></div>
 </center>
  </body>
</html>
