...
<hr/>
	<c:forEach items="${multiples}" var="multiple" varStatus="status" step="1" >
		<div id="showMultipleItem" class="defineTop" tip="${status.count}">
			<div class="defineTitle"><strong>第${status.count}题</strong></div>
			<div class="defineTitle">
					<input type="hidden" name="flag" flag="false"/>
					<c:choose>
						<c:when test="${multiple.image=='defaultQuestBank.jpg'}">[当前试题无图]</c:when>
						<c:otherwise>
			...
			<div class="defineBase">
				<div>	
					<fieldset>
						<legend>内容：</legend>
						<textarea rows="4" cols="75" id="content" disabled="disabled">${multiple.content}</textarea>
					</fieldset>	
				</div>	
				<table>
				<c:forEach items="${multiple.choices}" var="result" varStatus="statusT" step="1" >
					<tr>
						<td>${statusT.count}</td>
						<td><input type="checkbox" name="results" value="${multiple.results[statusT.count-1] }" ${multiple.results[statusT.count-1]=='1'?'checked':''  }/></td>
						<td style="text-align:left;">${fn:escapeXml(multiple.choices[statusT.count-1])}</td>
					</tr>
				</c:forEach>
				</table>
			</div>
		...
		<br/><br/>
			...
			<div><a href="###" onclick="isSaveMultiple('1')" title="保存">保存试题</a></div>
 </center>
  </body>
</html>
