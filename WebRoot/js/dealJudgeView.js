	var dataTable ;
	function initJudgesTableByAjax(id){
		if(dataTable!=null){
		...
		dataTable = $("#eachRow").dataTable( {
			...
			"sAjaxSource": "/OnLineTest/teacher/ajax/knowledge2judges?opvalue="+id,
			"bDestroy":true,
	...
	*/
	...
	function fnRowCallback( nRow, aData, iDisplayIndex){
			...
			$(nRow).css("cursor","pointer").click( function () { 
				...
				if(aData['id']!=bId){//同一行点击2次只加载一次
					loadJudgeTemplateDIV(aData['id']);
				}
			});
	}

	//加载的试题信息
	function loadJudgeTemplateDIV(questionID){
		var opvalue = questionID;
		// 设置与服务端进行交互的URL(包括中文处理),每次更改了知识点后就更新分页下标（由服务端更新）/teacher/ajax/knowledge2judges
		var url = "/OnLineTest/teacher/ajax/judgeId2Judge?opvalue=" + encodeURI(encodeURI(opvalue));
		url = covertURL(url);
		// 与服务端进行交互,并显示数据
	...
	//初始化或还原显示的试题信息
	function clearJudgeTemplateDIV(){
