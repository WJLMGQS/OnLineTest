  ...
  $.fn.extend({
    spin: function(o){
      return this.each(function(){
        ...
        var txt = $(this);
         /*自定义函数，用来完善输入框中出现的非法数据以及自动检测是否超出数据范围并校正*/
		 	
		  txt.change(function(){
        	var getInt = parseInt(txt.val());
        	//alert(txt.val());
        	if(isNaN(getInt)){
        		txt.val(this.defaultValue);
        	}else if( txt.val()<opt.min){
    			txt.val(opt.min);
    		}else if(txt.val()>opt.max){
    			txt.val(opt.max);
    		}else{
    			//txt.val(this.defaultValue);
    		}
