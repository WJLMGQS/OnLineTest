	var dataTable ;
	function initMultiplesTableByAjax(id){
		if(dataTable!=null){
		...
		dataTable = $("#eachRow").dataTable( {
			...
			"sAjaxSource": "/OnLineTest/teacher/ajax/knowledge2multiples?opvalue="+id,
			"bDestroy":true,
	...
	function fnRowCallback( nRow, aData, iDisplayIndex){
			...
			$(nRow).css("cursor","pointer").click( function () { 
				...
				if(aData['id']!=bId){//同一行点击2次只加载一次
					loadMultipleTemplateDIV(aData['id']);
				}
			});
	}

	//加载的试题信息
	function loadMultipleTemplateDIV(questionID){
		var opvalue = questionID;
		// 设置与服务端进行交互的URL(包括中文处理),每次更改了知识点后就更新分页下标（由服务端更新）/teacher/ajax/knowledge2multiples
		var url = "/OnLineTest/teacher/ajax/multipleId2Multiple?opvalue=" + encodeURI(encodeURI(opvalue));
		url = covertURL(url);
		// 与服务端进行交互,并显示数据
		$.getJSON(url, null, function(data) {
			$("#bId").val(questionID);
			$("#content").html(data.content);
			$("#checkBoxTable").html("<tr><td width='36'>序号</td><td>选项内容</td><td width='36'>答案</td></tr>");//这里让多选内容清空
			//添加多选项
			for(var i=0;i<data.choices.length;i++){
				var isChecked = data.results[i]==1?"checked":"";
				var tr = '<tr><td style="text-align:center">'+(i+1)+'</td><td><input type="text" value='+data.choices[i]+' disabled="disabled" /></td><td style="text-align:center"><input type="checkbox" name="TK" '+isChecked+'/></td></tr>';
			...
			$("#floatImage").attr("title","多选试题图片");
		});
	}
	//初始化或还原显示的试题信息
	function clearMultipleTemplateDIV(){
		$("#bId").val("0");//这里填0，为了和第一点击时的Id对比,记录试题的编号
		$("#content").html("此处尚未加载内容");
		$("#checkBoxTable").html("<tr><td width='36'>序号</td><td>选项内容</td><td width='36'>答案</td></tr>");
		$("#difficulty").val("未知");
