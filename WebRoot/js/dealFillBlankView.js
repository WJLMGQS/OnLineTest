	var dataTable ;
	function initFillBlanksTableByAjax(id){
		if(dataTable!=null){
		...
		dataTable = $("#eachRow").dataTable( {
			...
			"sAjaxSource": "/OnLineTest/teacher/ajax/knowledge2fillBlanks?opvalue="+id,
			"bDestroy":true,
	...
	*/
	...
	function fnRowCallback( nRow, aData, iDisplayIndex){
			...
			$(nRow).css("cursor","pointer").click( function () { 
				...
				if(aData['id']!=bId){//同一行点击2次只加载一次
					loadFillBlankTemplateDIV(aData['id']);
				}
			});
	}

	//加载的试题信息
	function loadFillBlankTemplateDIV(questionID){
		var opvalue = questionID;
		// 设置与服务端进行交互的URL(包括中文处理),每次更改了知识点后就更新分页下标（由服务端更新）/teacher/ajax/knowledge2fillBlanks
		var url = "/OnLineTest/teacher/ajax/fillBlankId2fillBlank?opvalue=" + encodeURI(encodeURI(opvalue));
		url = covertURL(url);
		// 与服务端进行交互,并显示数据
		$.getJSON(url, null, function(data) {
			$("#bId").val(questionID);
			$("#content").html(data.content);
			$("#fillBlankTable").html("<tr><td width='36'>序号</td><td>答案</td></tr>");//这里让多选内容清空
			//添加多选项
			for(var i=0;i<data.results.length;i++){
				var tr = '<tr><td>'+(i+1)+'</td><td><input type="text" value="'+data.results[i]+'" disabled="disabled"/></td></tr>';
				$(tr).appendTo("#fillBlankTable"); 
			}
	...
	//初始化或还原显示的试题信息
	function clearFillBlankTemplateDIV(){
		$("#bId").val("0");//这里填0，为了和第一点击时的Id对比,记录试题的编号
		$("#content").html("此处尚未加载内容");
		$("#fillBlankTable").html("<tr><td width='36'>序号</td><td>答案</td></tr>");
		$("#difficulty").val("未知");
		$("#analyse").html("此处尚未加载内容");
		$("#createTime").val("未知");
